...
class RegL2:
    ...
    def regularization_loss(self, layer):
        ...
        if self.kernel_regularizer > 0:
            regularization_loss += self.kernel_regularizer * np.sum(layer.weights * layer.weights)

        if self.bias_regularizer > 0:
            regularization_loss += self.bias_regularizer * np.sum(layer.biases * layer.biases)
    ...
    def regularization_layer(self, layer):
        ...
        # L2 on weights
        if self.kernel_regularizer > 0:
            dweights += 2 * self.kernel_regularizer * layer.weights

        # L2 on biases
        if (self.bias_regularizer > 0):
            dbiases += 2 * self.bias_regularizer * layer.biases
